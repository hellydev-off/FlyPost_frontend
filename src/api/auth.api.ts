...
export const authApi = {
  async telegram(initData: string, utmSource?: string, referralCode?: string): Promise<AuthResponse> {
    if (isMockMode) {
    ...
    const { data } = await api.post<AuthResponse>('/api/auth/telegram', { initData, utmSource, referralCode })
    return data
  },
